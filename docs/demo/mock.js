...
(function () {
  ...
  const post = (urn, publishedAt, hours, impressions, point, low, high, gain, tail) => ({ urn, publishedAt, hours, impressions, point, low, high, gainPerHour: gain, tailMode: tail, regime: 'calibrating', type: 'image', observedAt: new Date().toISOString() });
  const live = [
    post('7505901697601384448', new Date(Date.now() - 2.7 * 36e5).toISOString(), 2.7, 1017, 2207, 1690, 2980, 214, false),
    post('7505801697601384448', new Date(Date.now() - 6.1 * 36e5).toISOString(), 6.1, 3412, 4180, 3760, 4720, 96, false),
    post('7505701697601384448', new Date(Date.now() - 13.7 * 36e5).toISOString(), 13.7, 968, 1024, 990, 1090, 12, true),
  ];
  const state = {
